...
class Object():
    ...
    def updateDrawLine(self, p1, p2, viewport, window):
    ...
    def updateAndDrawObject(self, viewport, window):
        if self.tipo == 'ponto':
            ...
            self.updateDrawLine(self.pontos[0], self.pontos[1], viewport, window)
        elif self.tipo == 'wireframe':
            self.drawWireframe(viewport, window)
    
    def drawObject(self, viewport, window):
        if self.tipo == 'ponto':
            self.pontos[0].drawObject(viewport, window)
        elif self.tipo == 'reta':
            self.drawLine(self.pontos[0], self.pontos[1], viewport, window)
        elif self.tipo == 'wireframe':
            self.drawObject(viewport, window)



class Ponto(Object):
    def __init__(self, x, y, window):
        super().__init__([], 'dot', tipo='ponto', cor='black')
        self.x = x
        self.y = y
